...
char *id_kind_name2[]={"NULL","VAR","FUNC","PARM","FIELD","TYPE","ENUM","STRUCT","ENUM_LITERAL"};
char *spec_name2[]={"NULL","AUTO","STATIC","TYPEDEF"};
...
void print_space2(int);
void print_node2(A_NODE *,int);

void print_node2(A_NODE *node,int s){
		print_space2(s);
		printf("%s (%x,%d)\n",node_name2[node->name],node->type,node->value);
}
void print_space2(int s){
		int i;
		for(i=1;i<=s;i++)
				printf("| ");
}
			...
			printf("(DONE:%x)\n",t);
			 ...
			 switch (t->kind) {
				case T_ENUM:
					t->prt=FALSE;
					printf("ENUM\n");
					print_space2(s); printf("| ENUMERATORS\n");
					prt_sem_A_ID_LIST(t->field,s+2);
					break;
				case T_POINTER:
					t->prt=FALSE;
					printf("POINTER\n");
					print_space2(s); printf("| ELEMENT_TYPE\n");
					prt_sem_A_TYPE(t->element_type,s+2);
					break;
				case T_ARRAY:
					t->prt=FALSE;
					printf("ARRAY\n");
					print_space2(s); printf("| INDEX\n");
				...
				case T_FUNC:
					t->prt=FALSE;
					printf("FUNCTION\n");
					print_space2(s); printf("| PARAMETER\n");
					prt_sem_A_ID_LIST(t->field,s+2);
					print_space2(s); printf("| TYPE\n");
					prt_sem_A_TYPE(t->element_type,s+2);
					if (t->expr) {
						print_space2(s); printf("| BODY\n");
...
{
		print_space2(s);
		printf("(ID=\"%s\") TYPE:%x KIND:%s SPEC=%s LEV=%d VAL=%d ADDR=%d\n", id->name, id->type, id_kind_name2[id->kind], spec_name2[id->specifier],id->level, id->value, id->address);
}
